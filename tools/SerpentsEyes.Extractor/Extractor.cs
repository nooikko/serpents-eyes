...
internal static partial class Extractor
{
    ...
    public static int Run(string contentRoot)
    {
        ...
        var questFlavor = QuestFlavor.Build(tableEntries, quests);

        string generatedPath = ExtractorPaths.GeneratedTagDatabase;
        File.WriteAllText(generatedPath, GenerateCSharp(entriesOut, mapTitles, gods, lockRules, quests, questFlavor));
    ...
    private static string GenerateCSharp(List<TagEntry> entries, List<(string Key, string Title)> mapTitles,
        ...
        List<QuestFlavorEntry> questFlavor)
    {
        ...
        sb.AppendLine("    ];");
        sb.AppendLine();
        sb.AppendLine("    private static readonly (string Tag, string Text)[] QuestFlavorEntries =");
        sb.AppendLine("    [");
        foreach (var f in questFlavor)
        {
            sb.Append("        (").Append(Lit(f.Tag)).Append(", ").Append(Lit(f.Text)).AppendLine("),");
        }
        sb.AppendLine("    ];");
        sb.AppendLine("}");
        return sb.ToString();
